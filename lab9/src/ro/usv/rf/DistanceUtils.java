...
public class DistanceUtils {
	...
	protected static double euclidianDistanceGeneralized(String[] firstCharacteristic, String[] secondCharacteristic) {
		double sum =0;
		for(int i=0; i<firstCharacteristic.length;i++) {
//			System.out.println(Double.valueOf(firstCharacteristic[i]) +  firstCharacteristic[i] + " " + secondCharacteristic[i]);
			try {
				sum += Math.pow(Double.valueOf(firstCharacteristic[i]) - Double.valueOf(secondCharacteristic[i]), 2);				
