...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		try {
			...
			int desiredClassesNumber = 3;

			System.out.println(String.format("The learning set has %s patters and %s features", numberOfPatterns,
					numberOfFeatures));
		
			int[] classes = new int[learningSet.length];
			int[] oldClasses = new int[learningSet.length];
			for(int i=0;i<oldClasses.length;i++) {
				oldClasses[i] = -1; //need to initial have different arrays
			}
			int[] kernels = new int[desiredClassesNumber];
			for(int i=0;i<kernels.length;i++) {
				kernels[i] = i+1;
			}
			
			while(!isSimilar(oldClasses, classes)) {
				for(int i=0; i<oldClasses.length;i++) {
					oldClasses[i] = classes[i];
					classes[i] = getNearestClass(learningSet, kernels, learningSet[i]);
				}
				
				for(int i=0; i<desiredClassesNumber;i++) {					
					kernels[i] = getNewKernel(learningSet, classes, i+1); 
				}					
			}
...
//			}
	...
	private static boolean isSimilar(int[] oldClasses, int[] newClasses) {
		for(int i=0; i<oldClasses.length; i++) {
	...
	private static int getNearestClass(String[][] learningSet, int[] kernels, String[] feature) {
		double minDistance = DistanceUtils.euclidianDistanceGeneralized(feature, learningSet[kernels[0]-1]);
		int nearest = 0;
		
		for(int i=1; i<kernels.length;i++) {
			double dist = DistanceUtils.euclidianDistanceGeneralized(feature, learningSet[kernels[i]-1]);
			if(dist<minDistance) {
				minDistance = dist;
				nearest = i;
			}
	...
	private static int getNewKernel(String[][] learningSet, int[] classes, int classNumber) {
		double[] center = new double[learningSet[0].length];
		String[] centerStr = new String[learningSet[0].length];
		int nr = 0;
		
		for(int i=0; i<classes.length; i++) {
			if(classes[i] == classNumber) {
				for(int j=0; j<learningSet[0].length; j++) {
					center[j]+= Double.valueOf(learningSet[i][j]);
				}
				nr++;
			}
		}
		...
		for(int i=0; i<center.length; i++) {
			center[i] = center[i]/nr;
			centerStr[i] = center[i] + "";
			}
		...
		for(int i=0; i<classes.length; i++) {
			if(classes[i] == classNumber) {
				double dist = DistanceUtils.euclidianDistanceGeneralized(centerStr, learningSet[i]);
				if(dist<min) {
					min = dist;
					poz = i;
				}
